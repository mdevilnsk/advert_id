...
+ (AdvertIdPlugin *)sharedInstance {
    if (_instance == nil) {
        _instance = [[AdvertIdPlugin alloc] init];
    }
    return _instance;
}

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
    AdvertIdPlugin *instance = [AdvertIdPlugin sharedInstance];
...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    if ([@"getInitialLink" isEqualToString:call.method]) {
        ...
        if ([manager isAdvertisingTrackingEnabled]){
            NSUUID *adId = [manager advertisingIdentifier];
